...
**That's the part that's ending nowadays**. An AI agent querying your platform doesn't have a habit. It doesn't ask a colleague which table everyone uses, it defaults to searching the whole thing at once, suddenly finding the table with a plausible-sounding name, and uses it with full confidence, whether or not anyone still stands behind it. This could be a stale niche dataset or some raw data - nor being intended for general data consumers. The obscurity that used to protect you was never a safeguard. It was luck, and agents don't share the instincts that made the luck hold.
